...
// -----------------------------
// REPORT CUSTOMIZATION FUNCTIONALITY
// -----------------------------
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['customize_report'])) {
    if ($_SESSION['user']['role'] === 'admin') {
        $report_type = $_POST['report_type'];
        $columns = isset($_POST['columns']) ? $_POST['columns'] : [];
        $_SESSION['custom_reports'][$report_type] = $columns;
    }
}
...
if (isset($_GET['download']) && isset($_GET['type'])) {
    ...
    $custom_columns = isset($_SESSION['custom_reports'][$type]) ? $_SESSION['custom_reports'][$type] : [];

    if ($type == 'products') {
        $default_columns = array('ID', 'Name', 'Brand', 'Unit', 'Category', 'Description', 'Price', 'Stock', 'Created At');
        $columns = !empty($custom_columns) ? $custom_columns : $default_columns;
        fputcsv($output, $columns);

        $query = "SELECT " . implode(", ", array_map('strtolower', $columns)) . " FROM products";
        $result = $conn->query($query);
        while ($row = $result->fetch_assoc()) {
            fputcsv($output, array_intersect_key($row, array_flip(array_map('strtolower', $columns))));
        }
    } elseif ($type == 'rentals') {
...
<div class="container">
  ...
  <div class="mb-4">
    ...
    <form method="POST">
      ...
      <button type="submit" name="customize_report" class="btn btn-primary">Save Customization</button>
    </form>
  </div>
  <?php endif; ?>
